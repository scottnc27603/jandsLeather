...
//ie fix select width plug-in
/*global $j jQuery window */
	...
	selectFix = {
		...
		**/
		addOverlay : function (element)
		{
			...
			// Get the overlay.
			overlay = $j('a#' + overlayId);


			// Check for bgiframe for IE 6.
			if (!window.XMLHttpRequest && ($j.fn.bgIframe || $j.fn.bgiframe))
			{
				overlay.bgiframe();
			}
		...
		**/
		openSelect : function (event)
		{
			...
			overlay = $j('a#' + elementId + '-' + options.overlayClassName);
			pageX = event.pageX;
			pageY = event.pageY;
			type = event.type;
		...
		**/
		setClass : function (event)
		{
			...
			element = $j(event.target);
			offset = element.offset();
			overlay = $j('a#' + element.attr('id') + '-' + options.overlayClassName);
			pageX = event.pageX;
			pageY = event.pageY;
			type = event.type;
